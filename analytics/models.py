...
#     filename = models.CharField(max_length=255)
#     uploaded_at = models.DateTimeField(auto_now_add=True)
#     status = models.CharField(max_length=25, choices=UPLOAD_STATUS_CHOICES, default='pending')
#     processed_rows = models.IntegerField(default=0)
#     total_rows = models.IntegerField(default=0)
#     success_count = models.IntegerField(default=0)
#     error_count = models.IntegerField(default=0)
#     error_log = models.TextField(blank=True, null=True)
#     csv_headers = models.JSONField(default=list, blank=True)  # Store actual CSV headers
#
#     class Meta:
#         ordering = ['-uploaded_at']
#
...
#     title = models.CharField(max_length=255)
#     external_id = models.CharField(max_length=255, blank=True, null=True)
#     upc = models.CharField(max_length=20, blank=True, null=True)  # Universal Product Code
#     release_date = models.DateField(blank=True, null=True)
#     created_at = models.DateTimeField(auto_now_add=True)
#
#     class Meta:
...
#     name = models.CharField(max_length=255)
#     external_id = models.CharField(max_length=255, blank=True, null=True)
#     isrc = models.CharField(max_length=15, blank=True, null=True)  # International Standard Recording Code
#     duration_seconds = models.IntegerField(blank=True, null=True)
#     track_number = models.IntegerField(blank=True, null=True)
#     created_at = models.DateTimeField(auto_now_add=True)
#
...
#     track = models.ForeignKey(Track, on_delete=models.CASCADE, related_name='royalty_statements')
#     platform = models.ForeignKey(Platform, on_delete=models.CASCADE, related_name='royalty_statements')
#     upload = models.ForeignKey(CsvUpload, on_delete=models.CASCADE, related_name='statements', null=True, blank=True)
#
#     # Date fields
#     period_start = models.DateField()
#     period_end = models.DateField()
#     report_date = models.DateField(blank=True, null=True)  # When the report was generated
#
#     # Metrics
#     streams = models.IntegerField(default=0, validators=[MinValueValidator(0)])
#     downloads = models.IntegerField(default=0, validators=[MinValueValidator(0)])
#     revenue = models.DecimalField(max_digits=14, decimal_places=6, default=0.0)  # Increased precision
#     usd_revenue = models.DecimalField(max_digits=14, decimal_places=6, default=0.0)  # Converted to USD
#
#     # Additional fields for comprehensive data
#     territory = models.CharField(max_length=3, default='US')  # ISO country code
#     currency = models.CharField(max_length=3, default='USD')
#     quantity = models.IntegerField(default=0)  # For units sold/streamed
#     unit_price = models.DecimalField(max_digits=10, decimal_places=6, default=0.0)
#
#     # Metadata
#     source_row_hash = models.CharField(max_length=64, unique=True)
#     original_data = models.JSONField(default=dict, blank=True)  # Store original CSV row data
#     created_at = models.DateTimeField(auto_now_add=True)
...
#             models.Index(fields=['artist', 'period_end']),
#             models.Index(fields=['platform', 'period_end']),
#             models.Index(fields=['track', 'period_end']),
#             models.Index(fields=['territory', 'period_end']),
#         ]
#         unique_together = ['artist', 'track', 'platform', 'period_end', 'territory']
#
#     def __str__(self):
#         return f"Royalty: {self.track.name} on {self.platform.name} ({self.period_end})"
#
#     def save(self, *args, **kwargs):
#         import hashlib
#         if not self.source_row_hash:
#             source_string = f"{self.artist_id}-{self.track_id}-{self.platform_id}-{self.period_end}-{self.territory}-{self.streams}-{self.revenue}"
#             self.source_row_hash = hashlib.sha256(source_string.encode()).hexdigest()
#         super().save(*args, **kwargs)
#
#     @property
#     def revenue_per_stream(self):
#         if self.streams > 0:
#             return self.revenue / self.streams
#         return 0